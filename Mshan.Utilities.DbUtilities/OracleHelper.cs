//------------------------------------------------------------
// All Rights Reserved , Copyright (C) 2010 , Mshan , Ltd. 
//------------------------------------------------------------
...
namespace Mshan.Utilities.DbUtilities
{
    ...
    public class OracleHelper : BaseDbHelper, IDbHelper
    {
        ...
        public IDbDataParameter MakeParameter(string parameterName, object parameterValue, DbType dbType, Int32 parameterSize, ParameterDirection parameterDirection)
        {
            ...
            if (parameterSize > 0)
            {
                parameter = new OracleParameter(parameterName, ConvertToSqlDbType(dbType), parameterSize);
            }
            else
            {
                parameter = new OracleParameter(parameterName, ConvertToSqlDbType(dbType));
            }
        ...
        #region  private OracleType ConvertToSqlDbType(System.Data.DbType dbType) 类型转换
        /// <summary>
        /// 类型转换
        /// </summary>
        /// <param name="dbType">数据类型</param>
        /// <returns>转换结果</returns>
        private OracleType ConvertToSqlDbType(System.Data.DbType dbType)
        {
            OracleParameter sqlParameter = new OracleParameter();
            sqlParameter.DbType = dbType;
            return sqlParameter.OracleType;
        }
        #endregion
